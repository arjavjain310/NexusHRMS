import { getOpenAIChat, getChatModel, isAIEnabled, isChatAIEnabled } from "./openai";
...
function mockAssistantResponse(userMessage) {
  ...
  if (lower.includes("resume") || lower.includes("candidate")) {
    return "Upload resumes in Resume AI Screening. The system extracts skills, scores match %, and auto-shortlists top candidates. Set OPENROUTER_RESUME_API_KEY for semantic matching.";
  }
  ...
  return `I'm Nexus AI (demo mode). You asked: "${userMessage.slice(0, 100)}${userMessage.length > 100 ? "..." : ""}". Add OPENROUTER_CHAT_API_KEY for full AI responses. I can help with leave, recruitment, interviews, payroll, and HR policies.`;
}
export { isAIEnabled, isChatAIEnabled };